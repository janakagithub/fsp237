#!/usr/bin/env python3
"""Force ETC to be driven by NADH oxidation, not by dihydroorotate dehydrogenase.

Blocks three reactions running in biologically-wrong directions as free NADH sinks:
  rxn00543_m0  ethanol dehydrog. mito  -> block entirely (mito alcohol fermentation
                                          shouldn't be a NADH dump under aerobic
                                          minimal-glucose growth)
  rxn00248_m0  malate dehydrog. mito   -> forward-only (canonical TCA direction)
  rxn00198_m0  citrate synthase        -> forward-only (irreversible in real biology)

Re-optimizes after each fix, reports biomass + ETC state + NADH balance changes.
Saves model + flux TSV + repainted Escher map.
"""
import json
import os
import re as _re
...
FIX_PLAN = [
    ('block mito ethanol dehydrog. (NADH dump)',
     [('rxn00543_m0', (0.0, 0.0))]),
    ('force MDH forward-only (canonical TCA)',
     [('rxn00248_m0', (0.0, 1000.0))]),
    ('force citrate synthase forward-only',
     [('rxn00198_m0', (0.0, 1000.0))]),
]

ETC_RXNS = [
    ('frxn13726_m0', 'Complex I (proton-pumping)'),
    ('rxn09523_m0',  'NDI (mito NADH dehydrog.)'),
    ('rxn09524_c0',  'NDE (cyto NADH dehydrog.)'),
    ('rxn09417_m0',  'Complex II / SDH'),
    ('frxn35348_m0', 'Complex III'),
    ('rxn35347_m0',  'Complex IV'),
    ('rxn08173_m0',  'ATP synthase'),
    ('rxn09563_c0',  'dihydroorotate dehydrog. (orotate path)'),
    ('EX_cpd00007_e0','O2 exchange'),
    ('EX_cpd00027_e0','Glucose exchange'),
    ('bio1',          'Biomass'),
]

def nadh_balance(model, sol, met_id):
    met = model.metabolites.get_by_id(met_id)
    prod, cons = [], []
    for r in model.reactions:
        c = r.metabolites.get(met, 0)
        f = sol.fluxes.get(r.id, 0)
        if c != 0 and abs(f) > 1e-9:
            v = c * f
            (prod if v > 1e-9 else cons if v < -1e-9 else None) and \
                (prod.append((v, r)) if v > 0 else cons.append((v, r)))
    return prod, cons
...
def report(model, sol, header):
    ...
    # NADH·m balance summary
    for met_id in ['cpd00004_m0', 'cpd00004_c0']:
        prod, cons = nadh_balance(model, sol, met_id)
        ptot = sum(v for v, _ in prod); ctot = sum(v for v, _ in cons)
        print(f'  {met_id} balance: +{ptot:.4f} produced / {ctot:.4f} consumed')
...
def main():
    ...
    sol = model.optimize()
    report(model, sol, 'BEFORE NADH-sink fixes')

    for label, fixes in FIX_PLAN:
        print(f'\n##### applying: {label} #####')
        ...
        if sol.objective_value is None or sol.objective_value < 1e-6:
            ...
            sys.exit(1)
        report(model, sol, f'AFTER: {label}')
...
<div id="compartment-legend" style="
  ...
  <b style="font-size:13px">fsp237 + ETC driven by NADH oxidation</b>
  <div style="margin:8px 0">{legend_html}</div>
  <i style="color:#999;font-size:11px">Intensity &amp; thickness = flux magnitude</i><br>
  <i style="color:#999;font-size:11px">mito ADH blocked &middot; MDH+CS forward-only &middot; biomass={g:.4f}</i>
</div>
"""
